...
#define OLIVEOS_FAT16_SIGNATURE 0x29
#define OLIVEOS_FAT16_FAT_ENTRY_SIZE 0x02
#define OLIVEOS_FAT16_BAD_SECTOR 0xFF7
#define OLIVEOS_FAT16_UNUSED 0x00

typedef unsigned int FAT_ITEM_TYPE;
#define FAT_ITEM_TYPE_DIRECTORY 0
#define FAT_ITEM_TYPE_FILE 1

// Fat directory entry attributes bimask
#define FAT_FILE_READ_ONLY 0x01
#define FAT_FILE_HIDDEN 0x02
#define FAT_FILE_SYSTEM 0x04
#define FAT_FILE_VOLUMN_LABEL 0x08
...
struct fat_private {
    ...
    struct disk_stream* cluster_read_steram;

    // Used to stream the file allocaiton table
...
static void fat16_init_private(struct disk* disk, struct fat_private* private) {
    ...
    private->cluster_read_steram = diskstreamer_new(disk->id);
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector) {
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != OLIVEOS_ALL_OK) {
        res = -EIO;
        goto out;
    }

    while(1) {
        if (diskstreamer_read(stream, &item, sizeof(item)) != OLIVEOS_ALL_OK) {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(
    struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory) {
        ...
        struct disk_stream* stream = fat_private->directory_stream;
        if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != OLIVEOS_ALL_OK) {
            res = -EIO;
            goto out_err;
        }

        if (diskstreamer_read(stream, dir, root_dir_size) != OLIVEOS_ALL_OK) {
            res = -EIO;
            goto out_err;
        }
...
int fat16_resolve(struct disk* disk) {
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != OLIVEOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != OLIVEOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
...
void fat16_to_proper_string(char** out, const char* in, size_t size) {
    ...
    while(*in != 0x00 && *in != 0x20) {
        ...
        *out += 1;
        ...
        // We cannot process anymore since we have exceeded the input buffer size
        if (i >= size - 1) {
            break;
        }
...
uint32_t fat16_get_first_fat_sectory(struct fat_private* private) {
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
    ...
    uint32_t fat_table_position = fat16_get_first_fat_sectory(private) * disk->sector_size;
    res = diskstreamer_seek(stream, fat_table_position * (cluster * OLIVEOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0) {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset) {
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        if (entry == OLIVEOS_FAT16_BAD_SECTOR) {
            res = -EIO;
            goto out;
        }
...
static int fat16_read_internal_from_stream(
    struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out) {
    ...
    if (total > 0) {
        ...
        res = fat16_read_internal_from_stream(
            disk, stream, cluster, offset+total_to_read, total, out + total_to_read);
    }
...
static int fat16_read_internal(struct disk* disk, int starting_cluster, int offset, int total, void* out) {
    ...
    struct disk_stream* stream = fs_private->cluster_read_steram;
...
struct fat_item* fat16_find_item_in_directory(
    struct disk* disk, struct fat_directory* directory, const char* name) {

        struct fat_item* f_item = 0;
        char tmp_filename[OLIVEOS_MAX_PATH];
