...
int task_switch(struct task* task) {
    current_task = task;
    paging_swtich(task->page_directory);
    return 0;
}
...
int task_init(struct task* task, struct process* process) {
    ...
    task->registers.ip = OLIVEOS_PROGRAM_VIRTUAL_ADDRESS;
    ...
    task->registers.esp = OLIVEOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
...
void* task_get_stack_item(struct task* task, int index) {
    ...
    // Swtich to the given tasks page
    task_page_task(task);
