...
static struct process* processes[OLIVEOS_MAX_PROCESSES] = {};
...
struct process* process_get(int process_id) {
    ...
    if (process_id < 0 || process_id >= OLIVEOS_MAX_PROCESSES) {
        return NULL;
    }
...
static int process_find_free_allocation_index(struct process* process) {
    ...
    int res = -ENOMEM;
    for (int i = 0; i < OLIVEOS_MAX_PROGRAM_ALLOCATION; i++) {
...
static struct process_allocation* process_get_allocation_by_addr(
    struct process* process, void* addr) {
...
void process_switch_to_any() {
    ...
    for (int i = 0; i < OLIVEOS_MAX_PROCESSES; i++) {
        if (processes[i]) {
...
void process_free(struct process* process, void* ptr_to_free) {
    ...
    // Unlinke the pages from the process for the given address
    struct process_allocation* allocation = 
        process_get_allocation_by_addr(process, ptr_to_free);
    if (!allocation) {
...
int process_map_binary(struct process* process) {
    ...
    paging_map_to(
        process->task->page_directory, 
        (void*)OLIVEOS_PROGRAM_VIRTUAL_ADDRESS, 
        process->ptr, 
...
int process_map_memory(struct process* process) {
    ...
    switch(process->filetype) {
        ...
        case PROCESS_FILETYPE_BINARY:
            ...
            break;
        default:
            panic("\nprocess_map_memory: Invalid Filetype");
    }
    ...
    // Finall map the stack
    paging_map_to(
        process->task->page_directory, 
        (void*)OLIVEOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, 
        process->stack, 
        paging_align_address(process->stack + OLIVEOS_USER_PROGRAM_STACK_SIZE),
        PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot) {
    ...
    program_stack_ptr = kzalloc(OLIVEOS_USER_PROGRAM_STACK_SIZE);
